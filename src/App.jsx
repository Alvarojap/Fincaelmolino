      ...
      :<div style={{padding:28,maxWidth:800}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr",gap:16,marginBottom:16}}>
          ...
          <div style={{background:T.olive,borderRadius:18,padding:20,color:T.ink}}>
            ...
            {(()=>{const total=getPrecioReserva(selActivo);const costes=Math.round(total*.35);const margen=total-costes;const pct=total>0?Math.round(margen/total*100):0;return<><div style={{fontSize:40,fontWeight:700,letterSpacing:-1.2,lineHeight:1}}>+{pct}%</div><div style={{fontSize:12,fontWeight:600,opacity:.8,marginTop:4}}>Margen {fE(margen)}</div><div style={{height:1,background:"rgba(26,26,26,.15)",margin:"12px 0"}}/><div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>{[["Limpieza",Math.round(total*.08)],["Jardín",Math.round(total*.06)],["Lavandería",Math.round(total*.04)],["Comisión",Math.round(total*.1)]].map(([l,v],i)=><div key={i} style={{background:"rgba(26,26,26,.08)",borderRadius:8,padding:"5px 8px"}}><div style={{fontSize:9,fontWeight:700,opacity:.7}}>{l}</div><div style={{fontSize:12,fontWeight:700,marginTop:2}}>{fE(v)}</div></div>)}</div></>;})()}
          </div>
        </div>
        {/* Contacto */}
