...
html{height:100%}
body{font-family:-apple-system,BlinkMacSystemFont,'Inter','Inter Tight','Segoe UI',sans-serif;background:#FAFAF7;color:#1A1714;min-height:100vh;-webkit-tap-highlight-color:transparent;-webkit-font-smoothing:antialiased}
...
.card{background:#FFFFFF;border-radius:16px;border:1px solid #E5E0D6;padding:14px;transition:all .15s ease}
...
.fi{width:100%;padding:12px 14px;border-radius:14px;border:1px solid #E5E0D6;background:#FFFFFF;color:#1A1A1A;font-size:14px;font-family:inherit;outline:none;transition:border-color .15s ease;-webkit-appearance:none;appearance:none;box-sizing:border-box}
...
.ov{position:fixed;inset:0;background:rgba(20,15,10,0.6);z-index:999;display:flex;align-items:flex-end;justify-content:center}
...
.modal{background:#F5F3F0;border:none;border-top-left-radius:24px;border-top-right-radius:24px;border-bottom-left-radius:0;border-bottom-right-radius:0;padding:24px 22px 34px;width:100%;max-height:92vh;overflow-y:auto;animation:modalSlideUp .24s cubic-bezier(.34,1.56,.64,1)}
...
@media (max-width:768px){
  ...
  /* Reserva espacio en la esquina superior derecha para el FAB hamburguesa
     en cualquier cabecera con padding-top:54 (los headers de pantalla).
     Así los botones de acción (+, campana, etc.) quedan a la izquierda
     del FAB en la misma fila visual, replicando el AppHeader del diseño.
     Se aplica al primer hijo directo de .main (la mayoría son fragments
     que React aplana). */
  .ph{padding-right:64px!important}
  .fm-screen-header{padding-right:64px!important}
  .main div[style*="padding: 54px 20px"]{padding-right:64px!important}
  .main div[style*="padding:54px 20px"]{padding-right:64px!important}
  .main div[style*="padding:'54px 20px"]{padding-right:64px!important}
  ...
  return <BadgeCtx.Provider value={{noVistos,refresh:refreshNoVistos}}>
    ...
    <div className="app">
      ...
      {/* FAB hamburguesa flotante (solo móvil) — abre el drawer lateral.
          Alineado verticalmente con los títulos de las pantallas (~54px de
          padding-top) para que quede inline con los botones de acción de
...
function JardinCheck({perfil,tok,rol}){
  ...
  // Aliases para el rediseño: template espera srvActivo/srvTareas/toggleTarea/toggleTareaSemanal/toggleTareaPuntual/finalizarServicio
  const srvActivo=misSrvs[0]||null;
  const srvTareas=(srvActivo?.jardin_servicio_tareas||[]);
  const toggleTarea=(t)=>toggleSrvTarea(t.id,t.done);
  const toggleTareaSemanal=(t)=>toggle(t.id,false);
  const toggleTareaPuntual=(t)=>toggle(t.id,true);
  const finalizarServicio=async()=>{
    if(!srvActivo||saving)return;setSaving(true);
    try{
      await sbPatch("jardin_servicios",`id=eq.${srvActivo.id}`,{estado:"completado"},tok);
      await load_();
    }catch(e){console.error("Error finalizando servicio jardín:",e);}
    setSaving(false);
  };
  ...
  return (
    <div style={{paddingBottom:100,background:T.bg,minHeight:'100%',fontFamily:T.sans}}>
      ...
      <div style={{padding:'54px 20px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <button onClick={()=>setPage&&setPage('dashboard')} style={{width:38,height:38,borderRadius:999,background:T.surface,border:'1px solid '+T.line,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
          <FmIcon name="chevL" size={16} stroke={T.ink}/>
        </button>
        <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:600}}>Jardín</div>
        <div style={{width:38}}/>
      </div>

      {/* Banner de verificación semanal (conservado: crítico para cerrar la semana) */}
      {bannerVerifJsx&&<div style={{padding:'0 20px 14px'}}>{bannerVerifJsx}</div>}

      {/* Si hay servicio activo: progreso + tareas */}
      {srvActivo?(
        <>
          <div style={{padding:'0 20px 14px'}}>
            <div style={{fontSize:26,fontWeight:800,color:T.ink,letterSpacing:-0.9,lineHeight:1.1}}>
              {srvActivo.nombre||srvActivo.titulo}
            </div>
            {(srvActivo.reserva_nombre||srvActivo.fecha_inicio)&&(
              <div style={{fontSize:13,color:T.ink3,marginTop:4,display:'flex',alignItems:'center',gap:6}}>
                <FmIcon name="calendar" size={13} stroke={T.olive}/>
                {srvActivo.reserva_nombre||`${new Date(srvActivo.fecha_inicio+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short'})}${srvActivo.fecha_fin?` – ${new Date(srvActivo.fecha_fin+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short'})}`:''}`}
              </div>
            )}
          </div>
          ...
          {/* Card progreso donut */}
          {(()=>{
            const total=srvTareas.length||0;
            const done=srvTareas.filter(t=>t.done).length||0;
            const pct=Math.round((done/Math.max(total,1))*100);
            return(
              <div style={{padding:'0 20px 14px'}}>
                <div style={{background:T.surface,borderRadius:20,padding:16,border:'1px solid '+T.line,display:'flex',alignItems:'center',gap:16}}>
                  <div style={{width:76,height:76,borderRadius:999,background:`conic-gradient(${T.olive} ${pct*3.6}deg,${T.line} 0)`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                    <div style={{width:60,height:60,borderRadius:999,background:T.surface,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
                      <div style={{fontSize:16,fontWeight:800,color:T.ink,letterSpacing:-0.4,lineHeight:1}}>{pct}%</div>
                      <div style={{fontSize:9,color:T.ink3,fontWeight:600}}>hecho</div>
                    </div>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:11,color:T.ink3,letterSpacing:0.8,textTransform:'uppercase',fontWeight:700,marginBottom:4}}>Tareas completadas</div>
                    <div style={{fontSize:24,fontWeight:800,color:T.ink,letterSpacing:-0.8,lineHeight:1}}>
                      {done}<span style={{fontSize:14,color:T.ink3,fontWeight:600}}>/{total}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {/* Lista tareas del servicio */}
          {srvTareas.length>0&&(
            <div style={{padding:'0 20px 14px'}}>
              <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:700,marginBottom:8}}>Tareas del servicio</div>
              <div style={{background:T.surface,borderRadius:18,border:'1px solid '+T.line,overflow:'hidden'}}>
                {srvTareas.map((t,i,arr)=>{
                  const done=!!t.done;
                  return(
                    <div key={t.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 16px',borderBottom:i<arr.length-1?'1px solid '+T.line:'none'}}>
                      <div onClick={()=>toggleTarea(t)} style={{width:22,height:22,borderRadius:7,background:done?T.olive:'transparent',border:'1.5px solid '+(done?T.olive:T.line),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,cursor:'pointer'}}>
                        {done&&<FmIcon name="check" size={12} stroke="white" sw={3}/>}
                      </div>
                      <span onClick={()=>toggleTarea(t)} style={{flex:1,fontSize:14,color:done?T.ink3:T.ink,textDecoration:done?'line-through':'none',fontWeight:done?500:600,cursor:'pointer'}}>
                        {t.txt||t.descripcion||t.nombre||t.tarea}
                      </span>
                      <span onClick={()=>openSrvNota(t)} style={{fontSize:14,cursor:'pointer',color:T.ink3,padding:4}}>{t.nota||t.foto_url?'✏️':'➕'}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Botón finalizar servicio si todo hecho */}
          {srvTareas.length>0&&srvTareas.every(t=>t.done)&&(
            <div style={{padding:'0 20px 14px'}}>
              <button onClick={finalizarServicio} disabled={saving} style={{width:'100%',padding:'16px 0',borderRadius:999,background:T.ink,color:'white',border:0,fontFamily:T.sans,fontWeight:700,fontSize:15,cursor:saving?'default':'pointer',opacity:saving?0.6:1,display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                <FmIcon name="check" size={16} stroke="white" sw={3}/>
        ...
        // Sin servicio activo: solo mostramos empty si tampoco hay tareas semanales
        (actv.length===0&&jpunt.length===0)&&(
          <div style={{padding:'40px 20px',textAlign:'center'}}>
            <div style={{fontSize:48,marginBottom:12}}>🌿</div>
            <div style={{fontSize:20,fontWeight:800,color:T.ink,letterSpacing:-0.5}}>Sin tareas esta semana</div>
            <div style={{fontSize:13,color:T.ink3,marginTop:6}}>Cuando haya trabajo asignado aparecerá aquí</div>
          </div>
        )
      )}

      {/* Control final del jardín (conservado: es la verificación semanal, flow crítico) */}
      {todoHecho&&!showFinal&&!isA&&!yaVerif&&(
        <div style={{padding:'0 20px 14px'}}>
          <button onClick={()=>{setFinalCheck({});setFinalMode(null);setFinalNota('');setShowFinal(true);}} style={{width:'100%',padding:'16px 0',borderRadius:999,background:T.olive,color:T.ink,border:0,fontFamily:T.sans,fontWeight:700,fontSize:15,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
            <FmIcon name="check" size={16} stroke={T.ink} sw={3}/>
            Abrir control final del jardín
          </button>
        </div>
      )}

      {/* Sección "Esta semana" — tareas semanales + puntuales */}
      {((actv||[]).length>0||(jpunt||[]).length>0)&&(
        <div style={{padding:'0 20px 14px'}}>
          <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:700,marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span>Esta semana · {TEMPORADA_LBL[temp]}</span>
            <span style={{color:T.olive,fontWeight:700}}>{comp}/{tot}</span>
          </div>
          <div style={{background:T.surface,borderRadius:18,border:'1px solid '+T.line,overflow:'hidden'}}>
            {[...(actv||[]).map(t=>({...t,__kind:'semanal'})),...(jpunt||[]).map(t=>({...t,__kind:'puntual'}))].map((t,i,arr)=>{
              const done=t.__kind==='semanal'?(sj?.[t.id]?.done||false):(t.done||false);
              const entry=t.__kind==='semanal'?(sj[t.id]||{}):t;
              return(
                <div key={(t.__kind||'')+(t.id||i)} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',borderBottom:i<arr.length-1?'1px solid '+T.line:'none'}}>
                  <div onClick={(e)=>{e.stopPropagation();t.__kind==='semanal'?toggleTareaSemanal(t):toggleTareaPuntual(t);}} style={{width:22,height:22,borderRadius:7,background:done?T.olive:'transparent',border:'1.5px solid '+(done?T.olive:T.line),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,cursor:'pointer'}}>
                    {done&&<FmIcon name="check" size={12} stroke="white" sw={3}/>}
                  </div>
                  <div onClick={(e)=>{e.stopPropagation();t.__kind==='semanal'?toggleTareaSemanal(t):toggleTareaPuntual(t);}} style={{flex:1,minWidth:0,cursor:'pointer'}}>
                    <div style={{fontSize:13,fontWeight:done?500:600,color:done?T.ink3:T.ink,textDecoration:done?'line-through':'none'}}>{t.txt}</div>
                    <div style={{fontSize:10,color:T.ink3,marginTop:2,display:'flex',alignItems:'center',gap:5,flexWrap:'wrap'}}>
                      {t.zona&&<span>{t.zona}</span>}
                      <span style={{padding:'1px 6px',borderRadius:999,background:t.__kind==='puntual'?T.terracotta+'22':T.olive+'22',color:t.__kind==='puntual'?T.terracotta:'#5A8A3E',fontWeight:600}}>
                        {t.__kind==='puntual'?'Puntual':(FREC_LBL[fr[t.id]||t.frec]||'Semanal')}
                      </span>
                    </div>
                    {entry.nota&&<div style={{fontSize:11,color:T.ink3,marginTop:4,padding:'4px 8px',background:T.bg,borderRadius:6}}>📝 {entry.nota}</div>}
                  </div>
                  <span onClick={()=>openNota(t.id,t.__kind==='puntual')} style={{fontSize:14,cursor:'pointer',color:T.ink3,padding:4}}>{entry.nota||entry.foto_url?'✏️':'➕'}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Servicios custom adicionales (si hay más de uno) */}
      {misSrvs.length>1&&misSrvs.slice(1).map(s=>{
        const tareas=s.jardin_servicio_tareas||[];const hechas=tareas.filter(t=>t.done).length;
        const abierto=srvOpen[s.id];
        return(
          <div key={s.id} style={{padding:'0 20px 14px'}}>
            <div style={{background:T.surface,borderRadius:18,border:'1px solid '+T.line,overflow:'hidden'}}>
              <div onClick={()=>setSrvOpen(prev=>({...prev,[s.id]:!prev[s.id]}))} style={{padding:'14px 16px',cursor:'pointer',display:'flex',alignItems:'center',gap:12}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:14,fontWeight:700,color:T.ink,letterSpacing:-0.2}}>🌿 {s.nombre}</div>
                  <div style={{fontSize:11,color:T.ink3,marginTop:2}}>{hechas}/{tareas.length} tareas</div>
                </div>
                <FmIcon name={abierto?'chevD':'chevR'} size={15} stroke={T.ink3}/>
              </div>
              {abierto&&tareas.length>0&&(
                <div style={{borderTop:'1px solid '+T.line,background:T.bg}}>
                  {tareas.map((t,j)=>{
                    const done=!!t.done;
                    return(
                      <div key={t.id} onClick={()=>toggleSrvTarea(t.id,t.done)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 16px',borderBottom:j<tareas.length-1?'1px solid '+T.line:'none',cursor:'pointer'}}>
                        <div style={{width:20,height:20,borderRadius:6,background:done?T.olive:'transparent',border:'1.5px solid '+(done?T.olive:T.line),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                          {done&&<FmIcon name="check" size={11} stroke="white" sw={3}/>}
                        </div>
                        <span style={{flex:1,fontSize:13,color:done?T.ink3:T.ink,textDecoration:done?'line-through':'none',fontWeight:done?500:600}}>{t.txt}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {/* Historial de semanas anteriores (conservado) */}
      {historial.length>0&&(
        <div style={{padding:'0 20px 14px'}}>
          <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:700,marginBottom:8}}>Semanas anteriores</div>
          {historial.map(h=><SemanaArchivada key={h.id} semana={h.semana} estado={h.done} nota={h.nota}/>)}
        </div>
      )}

      {/* Modal control final (conservado) */}
      {showFinal&&!isA&&(
...
// ─── JARDÍN ADMIN ────────────────────────────────────────────────────────────
function JardinAdmin({perfil,tok,setPage}){
  const cwk=wkKey();
  ...
  const [isDesktopJ,setIsDesktopJ]=useState(typeof window!=="undefined"&&window.innerWidth>=768);
  useEffect(()=>{const fn=()=>setIsDesktopJ(window.innerWidth>=768);window.addEventListener("resize",fn);return()=>window.removeEventListener("resize",fn);},[]);
  const {noVistos:nvJ}=useContext(BadgeCtx);
  ...
  const fmtEJ=v=>(Math.round(parseFloat(v)||0)).toLocaleString("es-ES")+"€";
  const srvSel=srvs.find(s=>s.id===selSrv);

  /* ═══════ DESKTOP LAYOUT ═══════ */
  if(isDesktopJ)return(
    <div style={{display:"flex",height:"100vh",background:T.bg,fontFamily:T.sans,overflow:"hidden",position:"fixed",inset:0,zIndex:1}}>

      {/* Sidebar negro */}
      <AppSidebarDesktop page="jadmin" setPage={setPage} perfil={perfil} noVistos={nvJ}/>
      ...
      <div style={{width:380,borderRight:"1px solid "+T.line,background:T.surface,display:"flex",flexDirection:"column",height:"100vh",flexShrink:0}}>
        ...
        <div style={{padding:"22px 20px 14px",borderBottom:"1px solid "+T.line,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              ...
              <div style={{fontSize:26,fontWeight:700,color:T.ink,letterSpacing:-.8,lineHeight:1.1,marginTop:2}}>Jardinería</div>
            </div>
            <button onClick={()=>{setSrvForm(srvVacio);setSrvTareas([]);setShowSrv(true);}} style={{height:36,padding:"0 14px",borderRadius:999,background:T.ink,color:"white",border:0,fontFamily:T.sans,fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
          ...
          <div style={{display:"flex",gap:2,background:T.bg,borderRadius:999,padding:3}}>
            {[
              ...
              <button key={t.k} onClick={()=>setTab(t.k)} style={{flex:1,padding:"7px 4px",borderRadius:999,border:0,cursor:"pointer",background:tab===t.k?T.ink:"transparent",color:tab===t.k?"#fff":T.ink2,fontWeight:700,fontSize:10.5,fontFamily:T.sans,whiteSpace:"nowrap"}}>
                {t.l}
              </button>
            ))}
          </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"8px 10px"}}>
          {tab==="semana"&&<>
            <div style={{fontSize:10,color:T.ink3,fontWeight:700,letterSpacing:1,textTransform:"uppercase",padding:"8px 4px 6px"}}>Tareas fijas · semana {cwk}</div>
            {actv.map(t=>{const e=sj[t.id]||{};return(
              <div key={t.id} style={{borderRadius:14,padding:"10px 13px",marginBottom:5,border:"1px solid "+T.line,background:e.done?T.olive+"08":"transparent"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:15,flexShrink:0}}>{e.done?"✅":"⬜"}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:T.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.txt}</div>
                    <div style={{fontSize:11,color:T.ink3,marginTop:2}}>{t.zona} · 🔁 {FREC_LBL[getFr(t)]}</div>
                  </div>
                </div>
              </div>
            );})}
            <div style={{fontSize:10,color:T.ink3,fontWeight:700,letterSpacing:1,textTransform:"uppercase",padding:"14px 4px 6px"}}>Puntuales</div>
            {jpunt.length===0&&<div style={{textAlign:"center",padding:"16px 0",color:T.ink3,fontSize:12}}>Sin puntuales</div>}
            {jpunt.map(t=>(
              <div key={t.id} style={{borderRadius:14,padding:"10px 13px",marginBottom:5,border:"1px solid "+T.line,background:t.done?T.olive+"08":"transparent"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:15,flexShrink:0}}>{t.done?"✅":"⬜"}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{t.txt}</div>
                    <div style={{fontSize:11,color:T.ink3,marginTop:2}}>{t.zona||"General"}</div>
                  </div>
                  <button onClick={()=>delPunt(t.id)} style={{background:"none",border:"none",color:"#F35757",cursor:"pointer",fontSize:13,padding:4}}>🗑</button>
                </div>
              </div>
            ))}
            <button onClick={()=>setShowM(true)} style={{width:"100%",padding:"10px 0",borderRadius:999,border:"1px dashed "+T.line,background:"transparent",color:T.ink3,fontFamily:T.sans,fontSize:12,fontWeight:600,cursor:"pointer",marginTop:8}}>+ Asignar puntual</button>
          </>}
          {tab==="servicios"&&<>
            {srvs.length===0&&<div style={{textAlign:"center",padding:"30px 0",color:T.ink3,fontSize:13}}>Sin servicios</div>}
            {srvs.map(s=>{
              const m=getOpsMeta(s.estado||"activo");
              const on=selSrv===s.id;
              const tareas=s.jardin_servicio_tareas||[];
              const hechas=tareas.filter(t=>t.done).length;
              const fi=new Date(s.fecha_inicio+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"short"});
              return(
                <div key={s.id} onClick={()=>setSelSrv(on?null:s.id)} style={{borderRadius:14,padding:"12px 13px",marginBottom:5,cursor:"pointer",background:on?T.ink:"transparent",border:"1px solid "+(on?T.ink2:T.line),transition:"all 0.1s"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    ...
                    <span style={{fontSize:11,fontWeight:600,color:on?"rgba(255,255,255,0.65)":T.ink3}}>{hechas}/{tareas.length} tareas</span>
                  </div>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <OpsAvatar name={s.jardinero_nombre||"?"} size={18}/>
                    <span style={{fontSize:11,color:on?"rgba(255,255,255,0.65)":T.ink3,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      {s.jardinero_nombre||"Sin asignar"} · {fi}
                    </span>
                  </div>
                </div>
              );
          ...
          {tab==="frec"&&<>
            {JARDIN_T[getTemporada()].map(t=>{
              ...
              return <div key={t.id} style={{padding:"10px 4px",borderBottom:"1px solid "+T.line}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                  <div style={{minWidth:0}}><div style={{fontSize:12,color:T.ink,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.txt}</div><div style={{fontSize:10,color:T.ink3}}>{t.zona}</div></div>
                  <div style={{display:"flex",gap:4,alignItems:"center",flexShrink:0}}>
                    <span style={{fontSize:9,padding:"2px 6px",borderRadius:999,background:activa?T.olive+"22":T.bg,color:activa?T.olive:T.ink3,fontWeight:700}}>{activa?"Activa":"No"}</span>
                    <button onClick={()=>setEditFr(ed?null:t.id)} style={{padding:"4px 8px",borderRadius:8,border:"1px solid "+T.line,background:T.surface,fontSize:10,fontWeight:600,color:T.ink2,cursor:"pointer",fontFamily:T.sans}}>🔁 {FREC_LBL[f]}</button>
                  </div>
                </div>
                {ed&&<div style={{marginTop:8,background:T.bg,borderRadius:8,padding:10,display:"flex",gap:4,flexWrap:"wrap"}}>{Object.entries(FREC_LBL).map(([v,l])=><button key={v} onClick={()=>setFrOv(t.id,v)} style={{padding:"5px 10px",borderRadius:999,border:parseInt(v)===f?"none":"1px solid "+T.line,background:parseInt(v)===f?T.ink:T.surface,color:parseInt(v)===f?"#fff":T.ink,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:T.sans}}>{l}</button>)}</div>}
              </div>;
            })}
          </>}
        </div>
      ...
      <div style={{flex:1,overflow:"auto",background:T.bg,minWidth:0}}>
        {!srvSel?(
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",flexDirection:"column",gap:14,color:T.ink3}}>
            <FmIcon name="leaf" size={44} stroke={T.line}/>
            <div style={{fontSize:15,fontWeight:500,color:T.ink3}}>Selecciona un servicio</div>
            <div style={{fontSize:12,color:T.ink3,opacity:.6}}>Verás aquí los detalles, tareas y progreso</div>
          </div>
        ):(()=>{
          const tareas=srvSel.jardin_servicio_tareas||[];
          const hechas=tareas.filter(t=>t.done).length;
          const fi=new Date(srvSel.fecha_inicio+"T12:00:00").toLocaleDateString("es-ES",{weekday:"short",day:"numeric",month:"long"});
          const ff=new Date(srvSel.fecha_fin+"T12:00:00").toLocaleDateString("es-ES",{weekday:"short",day:"numeric",month:"long"});
          const m=getOpsMeta(srvSel.estado||"activo");
          return(
          <div style={{padding:28,maxWidth:820}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22,gap:16}}>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,flexWrap:"wrap"}}>
                  ...
                  <span style={{fontSize:11,color:T.ink3,fontFamily:"monospace"}}>{String(srvSel.id).slice(-8)}</span>
                </div>
                <div style={{fontSize:32,fontWeight:700,color:T.ink,letterSpacing:-1,lineHeight:1.05}}>{srvSel.nombre}</div>
              </div>
              <div style={{display:"flex",gap:8,flexShrink:0}}>
                {srvSel.estado==="activo"&&hechas===tareas.length&&tareas.length>0&&<button onClick={()=>finalizarServicio(srvSel.id)} style={{padding:"9px 14px",borderRadius:11,border:0,background:T.olive,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:T.sans,color:"white",display:"flex",gap:6,alignItems:"center"}}>
                  <FmIcon name="check" size={13} stroke="white"/>Finalizar
                </button>}
                {srvSel.estado==="activo"&&<button onClick={()=>cancelarServicio(srvSel.id)} style={{padding:"9px 14px",borderRadius:11,border:"1px solid #FECACA",background:"#FEF2F2",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:T.sans,color:"#D9443A"}}>Cancelar</button>}
              </div>
            </div>

            {/* Grid: asignación + fechas */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
              ...
              <div style={{background:T.surface,borderRadius:18,padding:20,border:"1px solid "+T.line}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                  <OpsAvatar name={srvSel.jardinero_nombre||"?"} size={48}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:16,fontWeight:700,color:T.ink}}>{srvSel.jardinero_nombre||"Sin asignar"}</div>
                    <div style={{fontSize:12,color:T.ink3,marginTop:2}}>Jardinero</div>
                  </div>
                </div>
                <div style={{height:1,background:T.line,margin:"0 0 14px"}}/>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  <div>
                    <div style={{fontSize:10,color:T.ink3,textTransform:"uppercase",letterSpacing:.5,fontWeight:600}}>Inicio</div>
                    <div style={{fontSize:14,fontWeight:700,color:T.ink,marginTop:3}}>{fi}</div>
                  </div>
                  <div>
                    <div style={{fontSize:10,color:T.ink3,textTransform:"uppercase",letterSpacing:.5,fontWeight:600}}>Fin</div>
                    <div style={{fontSize:14,fontWeight:700,color:T.ink,marginTop:3}}>{ff}</div>
                  </div>
                </div>
              </div>
            ...
            {/* Notas */}
            {srvSel.notas&&<div style={{background:T.surface,borderRadius:18,padding:"14px 20px",border:"1px solid "+T.line,marginBottom:16,fontSize:13,color:T.ink,display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:16,flexShrink:0}}>📝</span>{srvSel.notas}
            </div>}

            {/* Tareas */}
            <div style={{background:T.surface,borderRadius:18,padding:20,border:"1px solid "+T.line,overflow:"hidden"}}>
              <div style={{fontSize:11,color:T.ink3,letterSpacing:.8,textTransform:"uppercase",fontWeight:700,marginBottom:14}}>
                Tareas · {hechas}/{tareas.length}
              </div>
              {tareas.length===0&&<div style={{color:T.ink3,fontSize:13,textAlign:"center",padding:"12px 0"}}>Sin tareas registradas</div>}
              {tareas.map((t,i)=>(
                <div key={t.id} style={{padding:"10px 0",display:"flex",alignItems:"flex-start",gap:12,borderBottom:i<tareas.length-1?"1px solid "+T.line:"none"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {t.foto_url&&<img src={t.foto_url} alt="" style={{marginTop:6,borderRadius:10,maxWidth:200,maxHeight:140}}/>}
...
// ─── LIMPIEZA CHECK (pantalla propia de la limpiadora, separada de Limpieza admin) ───
function LimpiezaCheck({perfil,tok,setPage}){
  const [srvActivoL,setSrvActivoL]=useState(null);
  const [tareasServ,setTareasServ]=useState([]);
  const [zonaExpandida,setZonaExpandida]=useState(null);
  const [showIncidencia,setShowIncidencia]=useState(false);
  const [incidenciaForm,setIncidenciaForm]=useState({zona:"",descripcion:"",foto:null,severidad:"media"});
  const [loading,setLoading]=useState(true);
  const [saving,setSaving]=useState(false);

  const targetId=perfil?.es_operario?perfil.referencia_id:perfil?.id;

  useEffect(()=>{
    (async()=>{
      try{
        // Servicios reales de limpieza: tabla 'servicios', estados activos según uso del admin existente
        const r=await sbGet("servicios",`?limpiadora_id=eq.${targetId}&estado=in.(programado,en_curso,activo,pendiente)&select=*&order=fecha.asc&limit=1`,tok).catch(()=>[]);
        if(r&&r.length>0){
          const srv=r[0];setSrvActivoL(srv);
          const t=await sbGet("servicio_tareas",`?servicio_id=eq.${srv.id}&select=*&order=zona.asc,id.asc`,tok).catch(()=>[]);
          setTareasServ(t||[]);
        }
      }catch(e){console.error("Error cargando servicio limpieza:",e);}
      setLoading(false);
    })();
  },[targetId,tok]);

  // Agrupar tareas por zona (schema real: servicio_tareas.zona es string)
  const zonasAgrupadas=(()=>{
    const grupos={};
    tareasServ.forEach(t=>{const zona=t.zona||"General";if(!grupos[zona])grupos[zona]=[];grupos[zona].push(t);});
    return Object.entries(grupos).map(([nombre,tareas])=>({
      nombre,tareas,
      todasHechas:tareas.length>0&&tareas.every(t=>t.done),
      hechas:tareas.filter(t=>t.done).length,
    }));
  })();
  const totalTareas=tareasServ.length;
  const tareasHechas=tareasServ.filter(t=>t.done).length;
  const pct=Math.round((tareasHechas/Math.max(totalTareas,1))*100);

  const toggleTarea=async(tarea)=>{
    if(saving)return;setSaving(true);
    const newDone=!tarea.done;
    try{
      await sbPatch("servicio_tareas",`id=eq.${tarea.id}`,{
        done:newDone,
        completado_por:newDone?perfil.nombre:null,
        completado_ts:newDone?new Date().toISOString():null,
      },tok);
      setTareasServ(arr=>arr.map(t=>t.id===tarea.id?{...t,done:newDone,completado_por:newDone?perfil.nombre:null,completado_ts:newDone?new Date().toISOString():null}:t));
    }catch(e){console.error("Error toggle tarea:",e);}
    setSaving(false);
  };

  const finalizarServicio=async()=>{
    if(!srvActivoL||saving)return;setSaving(true);
    try{
      await sbPatch("servicios",`id=eq.${srvActivoL.id}`,{
        estado:"finalizado",
      ...
      try{
        const adms=await sbGet("usuarios","?rol=eq.admin&select=id",tok).catch(()=>[]);
        for(const a of adms)await sbPost("notificaciones",{para:a.id,txt:`✅ ${perfil.nombre} ha finalizado el servicio "${srvActivoL.nombre||"Limpieza"}"`},tok).catch(()=>{});
      }catch(_){}
      setSrvActivoL(null);setTareasServ([]);
      setPage&&setPage("dashboard");
    }catch(e){console.error("Error finalizando:",e);alert("No se pudo finalizar el servicio");}
    setSaving(false);
  };

  // Incidencia → se persiste como servicio_tarea extra con nota. Así aparece en el módulo Incidencias
  // del admin (que agrega servicio_tareas con nota IS NOT NULL).
  const guardarIncidencia=async()=>{
    if(!srvActivoL||!incidenciaForm.descripcion.trim()||saving)return;setSaving(true);
    try{
      const prefix={alta:"🔴 URGENTE",media:"🟡",baja:"🔵"}[incidenciaForm.severidad]||"";
      const notaTxt=`${prefix} ${incidenciaForm.descripcion.trim()}`;
      await sbPost("servicio_tareas",{
        servicio_id:srvActivoL.id,
        tarea_id:`incidencia_${Date.now()}`,
        zona:incidenciaForm.zona||"General",
        txt:`Incidencia reportada por ${perfil.nombre}`,
        es_extra:true,
        done:true,
        completado_por:perfil.nombre,
        completado_ts:new Date().toISOString(),
      ...
      try{
        const adms=await sbGet("usuarios","?rol=eq.admin&select=id",tok).catch(()=>[]);
        const msg=`⚠️ Incidencia en "${srvActivoL.nombre||"Limpieza"}" (${incidenciaForm.zona||"General"}): ${notaTxt}`;
        for(const a of adms)await sbPost("notificaciones",{para:a.id,txt:msg},tok).catch(()=>{});
      }catch(_){}
      setShowIncidencia(false);
      setIncidenciaForm({zona:"",descripcion:"",foto:null,severidad:"media"});
    }catch(e){console.error("Error creando incidencia:",e);}
    setSaving(false);
  };

  if(loading){
    return <div style={{padding:"80px 20px",textAlign:"center",color:T.ink3,fontFamily:T.sans}}>Cargando…</div>;
  }
  ...
  return (
    <div style={{paddingBottom:100,background:T.bg,minHeight:'100%',fontFamily:T.sans}}>
      ...
      <div style={{padding:'54px 20px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:600}}>Limpieza</div>
        <div style={{width:38}}/>
      </div>
      ...
      {!srvActivoL?(
        <div style={{padding:'40px 20px',textAlign:'center'}}>
          <div style={{fontSize:48,marginBottom:12}}>✨</div>
          <div style={{fontSize:20,fontWeight:800,color:T.ink,letterSpacing:-0.5}}>Sin servicio activo</div>
          <div style={{fontSize:13,color:T.ink3,marginTop:6}}>Cuando tengas un servicio asignado aparecerá aquí</div>
        </div>
      ):(
        <>
          {/* Título */}
          <div style={{padding:'0 20px 14px'}}>
            <div style={{fontSize:26,fontWeight:800,color:T.ink,letterSpacing:-0.9,lineHeight:1.1}}>
              {srvActivoL.nombre||srvActivoL.titulo||'Servicio de limpieza'}
            </div>
            {srvActivoL.fecha&&(
              <div style={{fontSize:13,color:T.ink3,marginTop:4,display:'flex',alignItems:'center',gap:6}}>
                <FmIcon name="calendar" size={13} stroke={T.terracotta}/>
                {new Date(srvActivoL.fecha+'T12:00:00').toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'})}
              </div>
            )}
          </div>
          ...
          <div style={{padding:'0 20px 14px'}}>
            <div style={{background:T.surface,borderRadius:20,padding:16,border:'1px solid '+T.line,display:'flex',alignItems:'center',gap:16}}>
              <div style={{width:76,height:76,borderRadius:999,background:`conic-gradient(${T.terracotta} ${pct*3.6}deg,${T.line} 0)`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                <div style={{width:60,height:60,borderRadius:999,background:T.surface,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
                  <div style={{fontSize:16,fontWeight:800,color:T.ink,letterSpacing:-0.4,lineHeight:1}}>{pct}%</div>
                  <div style={{fontSize:9,color:T.ink3,fontWeight:600}}>hecho</div>
                </div>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:11,color:T.ink3,letterSpacing:0.8,textTransform:'uppercase',fontWeight:700,marginBottom:4}}>Tareas completadas</div>
                <div style={{fontSize:24,fontWeight:800,color:T.ink,letterSpacing:-0.8,lineHeight:1}}>
                  {tareasHechas}<span style={{fontSize:14,color:T.ink3,fontWeight:600}}>/{totalTareas}</span>
                </div>
                <div style={{fontSize:11,color:T.ink3,marginTop:4}}>{zonasAgrupadas.filter(z=>z.todasHechas).length}/{zonasAgrupadas.length} zonas</div>
              </div>
            </div>
          </div>
          ...
          {zonasAgrupadas.length>0&&(
            <div style={{padding:'0 20px 14px'}}>
              <div style={{fontSize:11,color:T.ink3,letterSpacing:1,textTransform:'uppercase',fontWeight:700,marginBottom:8}}>Zonas a limpiar</div>
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {zonasAgrupadas.map((z,i)=>{
                  const expandida=zonaExpandida===z.nombre;
                  return(
                    <div key={z.nombre} style={{background:T.surface,borderRadius:16,border:'1px solid '+(z.todasHechas?T.olive+'44':T.line),overflow:'hidden'}}>
                      <div onClick={()=>setZonaExpandida(expandida?null:z.nombre)} style={{padding:'14px 16px',cursor:'pointer',display:'flex',alignItems:'center',gap:12}}>
                        <div style={{width:28,height:28,borderRadius:8,background:z.todasHechas?T.olive:T.line+'66',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:10,fontWeight:800,color:z.todasHechas?'white':T.ink3}}>
                          {z.todasHechas?<FmIcon name="check" size={13} stroke="white" sw={3}/>:String(i+1).padStart(2,'0')}
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:14,fontWeight:700,color:T.ink,letterSpacing:-0.2}}>{z.nombre}</div>
                          <div style={{fontSize:11,color:T.ink3,marginTop:2}}>{z.hechas}/{z.tareas.length} tareas</div>
                        </div>
                        <FmIcon name={expandida?'chevD':'chevR'} size={15} stroke={T.ink3}/>
                      </div>
                      {expandida&&(
                        <div style={{borderTop:'1px solid '+T.line,background:T.bg}}>
                          {z.tareas.map((t,j)=>{
                            const done=!!t.done;
                            return(
                              <div key={t.id} onClick={()=>toggleTarea(t)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 16px 11px 52px',borderBottom:j<z.tareas.length-1?'1px solid '+T.line:'none',cursor:'pointer'}}>
                                <div style={{width:20,height:20,borderRadius:6,background:done?T.olive:'transparent',border:'1.5px solid '+(done?T.olive:T.line),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                                  {done&&<FmIcon name="check" size={11} stroke="white" sw={3}/>}
                                </div>
                                <span style={{flex:1,fontSize:13,color:done?T.ink3:T.ink,textDecoration:done?'line-through':'none',fontWeight:done?500:600}}>
                                  {t.txt||(LIMP_T.find(x=>x.id===t.tarea_id)?.txt)||t.tarea_id||'Tarea'}
                                </span>
                              </div>
                            );
                          })}
          ...
          {/* Botón incidencia */}
          <div style={{padding:'0 20px 14px'}}>
            <button onClick={()=>setShowIncidencia(true)} style={{width:'100%',padding:'14px 0',borderRadius:16,background:T.surface,border:'1px dashed '+T.terracotta+'88',color:T.terracotta,fontFamily:T.sans,fontWeight:700,fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
              <FmIcon name="camera" size={15} stroke={T.terracotta}/>
              Reportar incidencia
            </button>
          </div>

          {/* Finalizar servicio */}
          {tareasHechas>=totalTareas&&totalTareas>0&&(
            <div style={{padding:'0 20px 14px'}}>
              <button onClick={finalizarServicio} disabled={saving} style={{width:'100%',padding:'16px 0',borderRadius:999,background:T.ink,color:'white',border:0,fontFamily:T.sans,fontWeight:700,fontSize:15,cursor:saving?'default':'pointer',opacity:saving?0.6:1,display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                <FmIcon name="check" size={16} stroke="white" sw={3}/>
                {saving?'Guardando…':'Finalizar servicio'}
      ...
      {/* Sheet incidencia */}
      {showIncidencia&&(
        <div onClick={()=>setShowIncidencia(false)} style={{position:'fixed',inset:0,background:'rgba(20,15,10,0.6)',zIndex:999,display:'flex',alignItems:'flex-end',fontFamily:T.sans}}>
          <div onClick={e=>e.stopPropagation()} style={{width:'100%',background:T.bg,borderTopLeftRadius:24,borderTopRightRadius:24,maxHeight:'92vh',overflow:'auto',paddingBottom:34}}>
            <div style={{padding:'14px 0 0',display:'flex',justifyContent:'center'}}>
              <div style={{width:44,height:4,borderRadius:999,background:T.line}}/>
            </div>
            <div style={{padding:'14px 20px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+T.line}}>
              <div style={{fontSize:20,fontWeight:800,color:T.ink,letterSpacing:-0.6}}>Nueva incidencia</div>
              <button onClick={()=>setShowIncidencia(false)} style={{width:32,height:32,borderRadius:999,background:T.surface,border:'1px solid '+T.line,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
                <FmIcon name="x" size={14} stroke={T.ink}/>
              </button>
            </div>
            <div style={{padding:'16px 20px',display:'flex',flexDirection:'column',gap:14}}>
              <div>
                <div style={{fontSize:10.5,color:T.ink3,fontWeight:700,letterSpacing:0.8,textTransform:'uppercase',marginBottom:8}}>Zona</div>
                <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                  {zonasAgrupadas.map(z=>{
                    const on=incidenciaForm.zona===z.nombre;
                    return(
                      <button key={z.nombre} onClick={()=>setIncidenciaForm(v=>({...v,zona:z.nombre}))} style={{padding:'7px 12px',borderRadius:999,border:'1px solid '+(on?T.ink:T.line),background:on?T.ink:T.surface,color:on?'white':T.ink2,fontFamily:T.sans,fontSize:12,fontWeight:700,cursor:'pointer'}}>
                        {z.nombre}
                      </button>
                    );
                  })}
              ...
              <div>
                <div style={{fontSize:10.5,color:T.ink3,fontWeight:700,letterSpacing:0.8,textTransform:'uppercase',marginBottom:8}}>Descripción</div>
                <textarea rows={4} value={incidenciaForm.descripcion} onChange={e=>setIncidenciaForm(v=>({...v,descripcion:e.target.value}))} placeholder="Describe qué has encontrado…" style={{width:'100%',padding:'12px 14px',boxSizing:'border-box',borderRadius:14,border:'1px solid '+T.line,background:T.surface,fontFamily:T.sans,fontSize:13,color:T.ink,fontWeight:500,resize:'none',outline:'none',lineHeight:1.5}}/>
              </div>
              <div>
                <div style={{fontSize:10.5,color:T.ink3,fontWeight:700,letterSpacing:0.8,textTransform:'uppercase',marginBottom:8}}>Urgencia</div>
                <div style={{display:'flex',gap:6}}>
                  {[{k:'baja',l:'Baja',c:T.softBlue},{k:'media',l:'Media',c:T.gold},{k:'alta',l:'Alta',c:T.terracotta}].map(s=>{
                    const on=incidenciaForm.severidad===s.k;
                    return(
                      <button key={s.k} onClick={()=>setIncidenciaForm(v=>({...v,severidad:s.k}))} style={{flex:1,padding:'10px 0',borderRadius:12,border:'1px solid '+(on?s.c:T.line),background:on?s.c:T.surface,color:on?T.ink:T.ink2,fontFamily:T.sans,fontSize:12,fontWeight:700,cursor:'pointer'}}>
                        {s.l}
                      </button>
                    );
                  })}
              ...
              <div style={{display:'flex',gap:8,paddingTop:4}}>
                <button onClick={()=>setShowIncidencia(false)} style={{flex:1,padding:'14px 0',borderRadius:999,border:'1px solid '+T.line,background:T.surface,color:T.ink,fontFamily:T.sans,fontWeight:600,fontSize:14,cursor:'pointer'}}>Cancelar</button>
                <button onClick={guardarIncidencia} disabled={saving||!incidenciaForm.descripcion.trim()} style={{flex:2,padding:'14px 0',borderRadius:999,border:0,background:T.ink,color:'white',fontFamily:T.sans,fontWeight:700,fontSize:14,cursor:(saving||!incidenciaForm.descripcion.trim())?'default':'pointer',opacity:(saving||!incidenciaForm.descripcion.trim())?0.5:1}}>{saving?'Guardando…':'Guardar incidencia'}</button>
              </div>
            </div>
          </div>
        </div>
...
// ─── DETALLE DE GASTO (overlay) ─────────────────────────────────────────────
// Replica el patrón GastoDetailScreen del diseño (carpeta finca el molino (7)/
// screens-admin-mobile.jsx): ColorBlock con categoría + importe, Card de
// información, vínculo a reserva (event/airbnb), adjuntar factura.
// Auto-gastos quedan en solo lectura con info banner; manuales se editan.
function GastoDetail({gasto,tok,perfil,onClose,onSaved,onDeleted}){
  const [g,setG]=useState(gasto);
  useEffect(()=>{setG(gasto);},[gasto?.id]);
  const esAuto=GASTO_AUTO.includes(g.origen);
  const catColors={Personal:T.lavender,personal:T.lavender,"Comisión gestor":T.terracotta,comision:T.terracotta,Suministros:T.softBlue,suministros:T.softBlue,Consumibles:T.olive,consumibles:T.olive,Mantenimiento:T.gold,mantenimiento:T.gold,Marketing:"#F2995E",marketing:"#F2995E",Otros:T.ink3,otros:T.ink3,"Material & Reposición":T.peach||"#F2995E"};
  const color=catColors[g.categoria]||T.ink3;
  const fE=v=>(Math.round(parseFloat(v)||0)).toLocaleString("es-ES")+"€";
  ...
  const [form,setForm]=useState({
    ...
    fecha:g.fecha||"",
    categoria:g.categoria||"Personal",
    notas:g.notas||"",
    reserva_id:g.reserva_vinculada_id||g.reserva_id||"",
    reserva_tipo:g.reserva_vinculada_tipo||(g.reserva_id?"evento":""),
  ...
  useEffect(()=>{
    ...
    sbGet("reservas","?select=id,nombre,fecha,incluye_casa&order=fecha.desc&limit=80",tok).then(setReservas).catch(()=>{});
    sbGet("reservas_airbnb","?select=id,huesped,fecha_entrada,fecha_salida&order=fecha_entrada.desc&limit=80",tok).then(setAirbnbs).catch(()=>{});
  },[showLinkPicker]);
  const [saving,setSaving]=useState(false);
  const [uploading,setUploading]=useState(false);
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      // Vinculación a reserva — intenta varias columnas posibles para tolerar
      // esquemas distintos (reserva_vinculada_id / reserva_id).
      const linkPatch={};
      if(form.reserva_id){linkPatch.reserva_vinculada_id=form.reserva_id;linkPatch.reserva_vinculada_tipo=form.reserva_tipo||"evento";}
      else{linkPatch.reserva_vinculada_id=null;linkPatch.reserva_vinculada_tipo=null;}
      if(form.factura_url!==undefined)linkPatch.factura_url=form.factura_url||null;
      // Patch principal — si alguna columna no existe, reintenta sin extras
      await sbPatch("gastos",`id=eq.${g.id}`,{...patch,...linkPatch},tok).catch(()=>
        sbPatch("gastos",`id=eq.${g.id}`,patch,tok).catch(()=>{})
      );
      const updated={...g,...patch,...linkPatch};
      setG(updated);setEdit(false);
      onSaved&&onSaved(updated);
    }catch(_){}
    setSaving(false);
  };
  const eliminar=async()=>{
    if(esAuto){alert("Este gasto es automático. Para eliminarlo modifica el servicio original.");return;}
    if(!window.confirm(`¿Eliminar el gasto "${g.concepto}"?`))return;
    try{await sbDelete("gastos",`id=eq.${g.id}`,tok);onDeleted&&onDeleted();}catch(_){}
  };
  const subirFactura=async(file)=>{
    if(!file)return;setUploading(true);
    try{
      const url=await uploadFotoSeguro(file);
      setForm(v=>({...v,factura_url:url}));
    }catch(_){}
    setUploading(false);
  };
  ...
  // Origen legible
  const origenInfo=(()=>{
    if(g.origen==="auto_comision")return{label:"Comisión automática",txt:"Generado al cobrar el saldo de una reserva con la comisión configurada."};
    if(g.origen==="auto_limpieza")return{label:"Servicio de limpieza",txt:"Generado al verificar un servicio. Para modificar el coste o las horas, edita el servicio en Limpieza."};
    if(g.origen==="auto_jardineria")return{label:"Servicio de jardinería",txt:"Generado al completar un servicio de jardín. Edítalo en Jardinería."};
    if(g.origen==="auto_recurrente")return{label:"Gasto recurrente",txt:"Replicado automáticamente cada mes. Edita el gasto plantilla para cambiar futuras ocurrencias."};
    return{label:"Entrada manual",txt:null};
  })();

  // Vínculo legible
  const vinculo=(()=>{
    const r=reservas.find(x=>String(x.id)===String(form.reserva_id));
    if(r)return{txt:r.nombre+(r.fecha?" · "+new Date(r.fecha+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"short"}):""),tipo:"evento"};
    const a=airbnbs.find(x=>String(x.id)===String(form.reserva_id));
    if(a)return{txt:`Airbnb · ${a.huesped}`,tipo:"airbnb"};
    if(form.reserva_id)return{txt:"Reserva vinculada",tipo:form.reserva_tipo||""};
    return null;
  })();

  return(
    <div style={{position:"fixed",inset:0,background:T.bg,zIndex:200,overflow:"auto",paddingBottom:120}}>
      {/* Header */}
      <div style={{padding:"54px 20px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <button onClick={onClose} style={{width:36,height:36,borderRadius:999,background:T.surface,border:`1px solid ${T.line}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><FmIcon name="chevL" size={16} stroke={T.ink}/></button>
        <div style={{fontSize:11,color:T.ink3,letterSpacing:.6,textTransform:"uppercase",fontWeight:700}}>Detalle gasto</div>
        <div style={{width:36}}/>
      </div>

      {/* ColorBlock con categoría */}
      <div style={{padding:"4px 20px 14px"}}>
        <div style={{background:color,borderRadius:22,padding:22,color:T.ink}}>
          <div style={{fontSize:11,color:"rgba(30,20,10,0.65)",letterSpacing:1,textTransform:"uppercase",fontWeight:700,marginBottom:6}}>{g.categoria||"—"}</div>
          <div style={{fontSize:18,fontWeight:700,letterSpacing:-.4,lineHeight:1.15,marginBottom:12}}>{g.concepto||"Sin concepto"}</div>
          <div style={{fontSize:40,fontWeight:800,letterSpacing:-1.8,lineHeight:.95,fontVariantNumeric:"tabular-nums"}}>{fE(g.importe)}</div>
          <div style={{marginTop:14,display:"flex",gap:6,flexWrap:"wrap"}}>
            <span style={{display:"inline-flex",alignItems:"center",height:22,padding:"0 9px",borderRadius:999,background:esAuto?T.ink:"rgba(30,20,10,0.18)",color:esAuto?"#fff":T.ink,fontSize:11,fontWeight:700}}>{esAuto?"Automático":"Manual"}</span>
            {g.recurrente&&<span style={{display:"inline-flex",alignItems:"center",height:22,padding:"0 9px",borderRadius:999,background:"rgba(30,20,10,0.18)",color:T.ink,fontSize:11,fontWeight:700}}>Recurrente</span>}
            {g.factura_url&&<span style={{display:"inline-flex",alignItems:"center",height:22,padding:"0 9px",borderRadius:999,background:"rgba(30,20,10,0.18)",color:T.ink,fontSize:11,fontWeight:700,gap:4}}><FmIcon name="mail" size={11} stroke={T.ink}/>Factura</span>}
            {vinculo&&<span style={{display:"inline-flex",alignItems:"center",height:22,padding:"0 9px",borderRadius:999,background:"rgba(30,20,10,0.18)",color:T.ink,fontSize:11,fontWeight:700}}>Vinculado</span>}
          </div>
        </div>
      </div>

      {/* Información — edición o solo lectura */}
      <div style={{padding:"0 20px 14px"}}>
        <div style={{background:T.surface,borderRadius:18,padding:16,border:`1px solid ${T.line}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:13,fontWeight:700,color:T.ink,letterSpacing:-.1,textTransform:"uppercase"}}>Información</div>
            {!edit&&!esAuto&&<button onClick={()=>setEdit(true)} style={{background:"transparent",border:`1px solid ${T.line}`,borderRadius:999,padding:"6px 12px",fontSize:11,fontWeight:700,color:T.ink2,cursor:"pointer",fontFamily:T.sans,display:"inline-flex",alignItems:"center",gap:5}}><FmIcon name="edit" size={11} stroke={T.ink2}/>Editar</button>}
          </div>
          {!edit?(
            <>
              {[
                ["Fecha",g.fecha?new Date(g.fecha+"T12:00:00").toLocaleDateString("es-ES",{weekday:"short",day:"numeric",month:"long",year:"numeric"}):"—"],
                ["Categoría",g.categoria||"—"],
                ["Origen",origenInfo.label],
                ["Reserva vinculada",vinculo?vinculo.txt:"—"],
                ["Factura",g.factura_url?"Adjuntada":"No adjuntada"],
                ["ID interno",String(g.id||"").slice(-8).toUpperCase()],
              ].map(([k,v],i,arr)=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderTop:i===0?"none":`1px solid ${T.line}`,fontSize:13,gap:10}}>
                  <span style={{color:T.ink3,fontWeight:500,flexShrink:0}}>{k}</span>
                  <span style={{color:T.ink,fontWeight:600,textAlign:"right",overflow:"hidden",textOverflow:"ellipsis"}}>{v}</span>
                </div>
              ))}
              {g.notas&&<div style={{marginTop:10,padding:"10px 12px",background:T.bg,borderRadius:12,fontSize:13,color:T.ink2,lineHeight:1.5}}>{g.notas}</div>}
            </>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div className="fg" style={{margin:0}}><label>Concepto</label><input className="fi" value={form.concepto} onChange={e=>setForm(v=>({...v,concepto:e.target.value}))}/></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div className="fg" style={{margin:0}}><label>Importe (€)</label><input type="number" inputMode="decimal" className="fi" value={form.importe} onChange={e=>setForm(v=>({...v,importe:e.target.value}))}/></div>
                <div className="fg" style={{margin:0}}><label>Fecha</label><input type="date" className="fi" value={form.fecha} onChange={e=>setForm(v=>({...v,fecha:e.target.value}))}/></div>
              </div>
              <div className="fg" style={{margin:0}}><label>Categoría</label>
                <select className="fi" value={form.categoria} onChange={e=>setForm(v=>({...v,categoria:e.target.value}))}>
                  {GASTO_CATS.map(c=><option key={c}>{c}</option>)}
                </select>
              </div>
              <div className="fg" style={{margin:0}}><label>Notas</label><textarea className="fi" rows={2} value={form.notas} onChange={e=>setForm(v=>({...v,notas:e.target.value}))} placeholder="Detalles adicionales…"/></div>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{padding:"0 20px 14px"}}>
        <div style={{background:T.surface,borderRadius:18,padding:16,border:`1px solid ${T.line}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontSize:13,fontWeight:700,color:T.ink,letterSpacing:-.1,textTransform:"uppercase"}}>Vincular a reserva</div>
            {form.reserva_id&&<button onClick={()=>setForm(v=>({...v,reserva_id:"",reserva_tipo:""}))} style={{background:"transparent",border:0,color:T.ink3,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:T.sans}}>Quitar</button>}
          </div>
          {vinculo?(
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:T.lavender+"14",border:`1px solid ${T.lavender}33`,borderRadius:12}}>
              <div style={{width:32,height:32,borderRadius:10,background:T.lavender,display:"flex",alignItems:"center",justifyContent:"center"}}><FmIcon name={vinculo.tipo==="airbnb"?"home":"calendar"} size={14} stroke={T.ink}/></div>
              <div style={{flex:1,minWidth:0,fontSize:13,fontWeight:600,color:T.ink}}>{vinculo.txt}</div>
              <button onClick={()=>setShowLinkPicker(true)} style={{background:"transparent",border:`1px solid ${T.line}`,borderRadius:999,padding:"5px 10px",fontSize:11,fontWeight:700,color:T.ink2,cursor:"pointer",fontFamily:T.sans}}>Cambiar</button>
            </div>
          ):(
            <button onClick={()=>setShowLinkPicker(true)} style={{width:"100%",padding:"12px 14px",borderRadius:14,border:`1px dashed ${T.line}`,background:T.bg,color:T.ink2,fontFamily:T.sans,fontWeight:600,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><FmIcon name="plus" size={14} stroke={T.ink2}/>Vincular a un evento o Airbnb</button>
          )}
          <div style={{fontSize:11,color:T.ink3,marginTop:8,lineHeight:1.5}}>Útil para imputar suministros (luz, agua) a una reserva concreta y ver su rentabilidad real.</div>
        </div>
      </div>
      ...
      <div style={{padding:"0 20px 14px"}}>
        <div style={{background:T.surface,borderRadius:18,padding:16,border:`1px solid ${T.line}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontSize:13,fontWeight:700,color:T.ink,letterSpacing:-.1,textTransform:"uppercase"}}>Factura</div>
            {form.factura_url&&<button onClick={()=>setForm(v=>({...v,factura_url:""}))} style={{background:"transparent",border:0,color:T.ink3,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:T.sans}}>Quitar</button>}
          </div>
          {form.factura_url?(
            <div style={{display:"flex",gap:12,alignItems:"center"}}>
              <a href={form.factura_url} target="_blank" rel="noreferrer" style={{textDecoration:"none",flex:1,display:"flex",gap:12,alignItems:"center"}}>
                <img src={form.factura_url} alt="Factura" style={{width:48,height:60,objectFit:"cover",borderRadius:8,border:`1px solid ${T.line}`,background:T.bg}}/>
                <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>Ver factura</div><div style={{fontSize:11,color:T.ink3,marginTop:2}}>Toca para abrir</div></div>
              </a>
            </div>
          ):(
            <label style={{width:"100%",padding:"12px 14px",borderRadius:14,border:`1px dashed ${T.line}`,background:T.bg,color:T.ink2,fontFamily:T.sans,fontWeight:600,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              {uploading?"⏳ Subiendo…":<><FmIcon name="camera" size={14} stroke={T.ink2}/>Adjuntar foto/PDF de factura</>}
              <input type="file" accept="image/*,application/pdf" style={{display:"none"}} onChange={e=>subirFactura(e.target.files?.[0])}/>
            </label>
          )}
        </div>
      </div>

      {/* Banner info para auto-gastos */}
      {esAuto&&origenInfo.txt&&<div style={{padding:"0 20px 14px"}}>
        <div style={{padding:14,background:T.gold+"22",border:`1px solid ${T.gold}55`,borderRadius:16,display:"flex",gap:10}}>
          <FmIcon name="info" size={18} stroke="#8A6B0F"/>
          <div style={{fontSize:12.5,color:"#6B5108",lineHeight:1.5,fontWeight:500}}>{origenInfo.txt}</div>
        </div>
      </div>}

      {/* Acciones inferiores */}
      <div style={{padding:"6px 20px 24px",display:"flex",gap:8}}>
        {edit?<>
          <button onClick={()=>{setEdit(false);setForm({concepto:g.concepto||"",importe:String(g.importe||""),fecha:g.fecha||"",categoria:g.categoria||"Personal",notas:g.notas||"",reserva_id:g.reserva_vinculada_id||g.reserva_id||"",reserva_tipo:g.reserva_vinculada_tipo||(g.reserva_id?"evento":""),factura_url:g.factura_url||""});}} style={{flex:1,padding:"14px 0",borderRadius:999,border:`1px solid ${T.line}`,background:T.surface,color:T.ink,fontFamily:T.sans,fontWeight:600,fontSize:14,cursor:"pointer"}}>Cancelar</button>
          <button onClick={guardar} disabled={saving} style={{flex:2,padding:"14px 0",borderRadius:999,border:0,background:T.ink,color:"#fff",fontFamily:T.sans,fontWeight:700,fontSize:14,cursor:"pointer"}}>{saving?"Guardando…":"Guardar cambios"}</button>
        </>:<>
          {/* Permitir guardar vinculación/factura sin entrar en edición */}
          {((form.reserva_id||"")!==(g.reserva_vinculada_id||g.reserva_id||"")||(form.factura_url||"")!==(g.factura_url||""))&&
            <button onClick={guardar} disabled={saving} style={{flex:2,padding:"14px 0",borderRadius:999,border:0,background:T.ink,color:"#fff",fontFamily:T.sans,fontWeight:700,fontSize:14,cursor:"pointer"}}>{saving?"Guardando…":"Guardar cambios"}</button>
          }
          {!esAuto&&<button onClick={eliminar} style={{flex:1,padding:"14px 0",borderRadius:999,border:`1px solid #FECACA`,background:"#FEF2F2",color:"#D9443A",fontFamily:T.sans,fontWeight:700,fontSize:14,cursor:"pointer"}}>Eliminar</button>}
        </>}
      </div>

      {/* Picker de reserva */}
      {showLinkPicker&&<div className="ov" onClick={()=>setShowLinkPicker(false)}>
        <div className="modal" style={{maxWidth:540,maxHeight:"82vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
          <h3>Vincular a reserva</h3>
          <input className="fi" placeholder="Buscar nombre o huésped…" value={resQuery} onChange={e=>setResQuery(e.target.value)} style={{marginBottom:12}}/>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            <div style={{fontSize:10,color:T.ink3,fontWeight:700,letterSpacing:.6,textTransform:"uppercase",margin:"4px 2px"}}>Eventos</div>
            {reservas.filter(r=>!resQuery||(r.nombre||"").toLowerCase().includes(resQuery.toLowerCase())).slice(0,15).map(r=>(
              <button key={"e"+r.id} onClick={()=>{setForm(v=>({...v,reserva_id:r.id,reserva_tipo:"evento"}));setShowLinkPicker(false);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,border:`1px solid ${T.line}`,background:String(form.reserva_id)===String(r.id)?T.lavender+"22":T.surface,cursor:"pointer",textAlign:"left",fontFamily:T.sans}}>
                <div style={{width:28,height:28,borderRadius:8,background:r.incluye_casa?T.terracotta+"33":T.gold+"33",display:"flex",alignItems:"center",justifyContent:"center"}}><FmIcon name="calendar" size={13} stroke={T.ink}/></div>
                <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,color:T.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.nombre}</div><div style={{fontSize:11,color:T.ink3}}>{r.fecha?new Date(r.fecha+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"short",year:"numeric"}):"Sin fecha"}{r.incluye_casa?" · Casa":""}</div></div>
              </button>
            ))}
            {reservas.length===0&&<div style={{padding:10,fontSize:12,color:T.ink3}}>Sin eventos</div>}
            <div style={{fontSize:10,color:T.ink3,fontWeight:700,letterSpacing:.6,textTransform:"uppercase",margin:"12px 2px 4px"}}>Airbnb</div>
            {airbnbs.filter(a=>!resQuery||(a.huesped||"").toLowerCase().includes(resQuery.toLowerCase())).slice(0,15).map(a=>(
              <button key={"a"+a.id} onClick={()=>{setForm(v=>({...v,reserva_id:a.id,reserva_tipo:"airbnb"}));setShowLinkPicker(false);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,border:`1px solid ${T.line}`,background:String(form.reserva_id)===String(a.id)?T.lavender+"22":T.surface,cursor:"pointer",textAlign:"left",fontFamily:T.sans}}>
                <div style={{width:28,height:28,borderRadius:8,background:T.softBlue+"33",display:"flex",alignItems:"center",justifyContent:"center"}}><FmIcon name="home" size={13} stroke={T.ink}/></div>
                <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,color:T.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.huesped}</div><div style={{fontSize:11,color:T.ink3}}>{a.fecha_entrada?new Date(a.fecha_entrada+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"short"}):"—"} → {a.fecha_salida?new Date(a.fecha_salida+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"short"}):"—"}</div></div>
              </button>
            ))}
            {airbnbs.length===0&&<div style={{padding:10,fontSize:12,color:T.ink3}}>Sin Airbnb</div>}
          </div>
          <div className="mft"><button className="btn bg" onClick={()=>setShowLinkPicker(false)}>Cancelar</button></div>
        </div>
      </div>}
    </div>
  );
